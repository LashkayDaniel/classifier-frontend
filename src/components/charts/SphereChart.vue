...
const xDomain = computed(() => {
  if (!spheres.value.length) return [0, 9]
...
function drawGrid(svg, x, y, size) {
  svg.append('g')
    ...
    .call(d3.axisBottom(x).ticks(size / 50).tickSize(-size).tickFormat(() => ''))

  svg.append('g')
    .attr('class', 'grid')
    .style('color', '#C0C0C0')
    .call(d3.axisLeft(y).ticks(size / 50).tickSize(-size).tickFormat(() => ''))
}
